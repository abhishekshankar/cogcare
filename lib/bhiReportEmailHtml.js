 ...
 * @param {{
 *   magicLinkUrl: string
 *   bookConsultMagicLinkUrl?: string
 *   email: string
...
export function buildBhiReportWithMagicLinkEmailHtml(results, options) {
  ...
  const bookBlock =
    typeof bookConsultMagicLinkUrl === 'string' && bookConsultMagicLinkUrl.length > 0
      ? `
<div style="font-family:sans-serif;color:#1A1A1A;max-width:560px;margin:0 auto 20px;padding:20px 24px;background:#EEF5F0;border-radius:12px;border:1px solid #B8D9C1;">
  <p style="margin:0 0 8px;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#2A5A3A;">Book a consultation</p>
  <p style="margin:0 0 14px;font-size:14px;color:#444;line-height:1.5;">After you sign in, you can schedule a 45-minute session with a cognitive health specialist. Use this link — same expiry as your dashboard link.</p>
  <a href="${escapeHtml(bookConsultMagicLinkUrl)}" style="display:inline-block;padding:14px 24px;background:#2A5A3A;color:#fff;text-decoration:none;border-radius:10px;font-weight:700;font-size:14px;">Book consultation</a>
</div>`
      : ''
