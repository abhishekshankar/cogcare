  ...
  return <section id="institution" className="rounded-[2rem] border border-border bg-white p-6 shadow-sm sm:p-8" aria-labelledby="institution-title">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      <div className={`${networkPanel} p-5`}><h3 className="font-serif text-lg text-ink">Recognition approvals</h3><p className={`mt-2 ${networkCaption}`}>Your name is not authorized by membership. Decide each proposed attribution separately.</p>
        {workspace.attributions?.filter((x) => x.status === 'pending').length ? <ul className={`mt-4 ${networkStackTight}`}>{workspace.attributions.filter((x) => x.status === 'pending').map((item) => <li key={item.id} className="border-t border-border pt-4"><p className={networkBodySm}>{item.proposedText}</p><div className="mt-3 flex gap-2"><button className={networkPrimaryBtn} disabled={busy} onClick={() => act('decideAttribution', { id: item.id, decision: 'approved' })}>Approve this item</button><button className={networkSecondaryBtn} disabled={busy} onClick={() => act('decideAttribution', { id: item.id, decision: 'declined' })}>Decline</button></div></li>)}</ul> : <p className={`mt-4 ${networkBodySm}`}>No attribution requests await your decision.</p>}
      </div>
      <div className={`${networkPanel} p-5`}><h3 className="font-serif text-lg text-ink">Professional introductions</h3><p className={`mt-2 ${networkCaption}`}>Contact details are not released here. Both members must consent first.</p>
        {workspace.introductions?.filter((x) => x.status === 'pending').length ? <ul className={`mt-4 ${networkStackTight}`}>{workspace.introductions.filter((x) => x.status === 'pending').map((item) => <li key={item.id} className="border-t border-border pt-4"><p className={networkBodySm}>{item.purpose}</p><div className="mt-3 flex gap-2"><button className={networkPrimaryBtn} disabled={busy} onClick={() => act('decideIntroduction', { id: item.id, decision: 'accepted' })}>Consent</button><button className={networkSecondaryBtn} disabled={busy} onClick={() => act('decideIntroduction', { id: item.id, decision: 'declined' })}>Decline quietly</button></div></li>)}</ul> : <p className={`mt-4 ${networkBodySm}`}>No introduction requests are pending.</p>}
      </div>
      ...
      <div className={`${networkPanel} p-5`}><h3 className="font-serif text-lg text-ink">Salons & controlled events</h3>{workspace.events?.length ? <ul className={`mt-4 ${networkStackTight}`}>{workspace.events.map((item) => <li key={item.id} className="border-t border-border pt-4"><strong className="text-sm text-ink">{item.title}</strong><p className={networkCaption}>{new Date(item.startsAt).toLocaleString()} · Attendance is private.</p><div className="mt-3 flex gap-2"><button className={networkPrimaryBtn} disabled={busy} onClick={() => act('respondEvent', { eventId: item.id, response: 'attending' })}>Request a place</button><button className={networkSecondaryBtn} disabled={busy} onClick={() => act('respondEvent', { eventId: item.id, response: 'declined' })}>Decline</button></div></li>)}</ul> : <p className={`mt-3 ${networkBodySm}`}>No salons or events are currently open.</p>}
      </div>
